...
fn main() {
    ...
    let handle_c1 = thread::spawn(move || {
        for received in rx1.iter() {
            println!("Child thread 1 received: {}", received);
        }
    });

    // Spawn the second child thread
    let rx2 = rx.clone();
    let handle_c2 = thread::spawn(move || {
        for received in rx2.iter() {
            println!("Child thread 2 received: {}", received);
        }
    });

    // Send messages from the main thread
    let values = vec!["Hello", "Rust", "World", "Concurrency"];
    for value in values {
